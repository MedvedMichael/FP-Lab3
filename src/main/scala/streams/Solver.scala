...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    b.legalNeighbors.foldLeft(LazyList[(Block, List[Move])]())((list, item) => (item._1, item._2 +: history) #:: list)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filter(item => !explored.contains(item._1) /*&& explored.size < 1000*/) //for infinite map
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    val next = initial.flatMap {
      case (block, path) =>
        newNeighborsOnly(
          neighborsWithHistory(block, path),
          explored
        )
    }
    if (next.isEmpty)
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] =
    pathsFromStart.filter(item => done(item._1))
  ...
  lazy val solution: List[Move] = {
    ...
    if (paths.isEmpty)
      ...
      paths.tail.foldLeft(paths.head._2)((best, curr) =>
        if (curr._2.length < best.length)
          curr._2
        else
          best
      )
